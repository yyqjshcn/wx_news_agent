...
import { Plus, Trash2, Edit2, Play, Clock, Loader2, ChevronDown } from "lucide-react";
...
export default function WorkflowsPage() {
  ...
  const fetchRuns = useCallback(async (workflowId: string, skip: number, limit: number) => {
    return await api.get(`/api/workflows/${workflowId}/runs?skip=${skip}&limit=${limit}`);
  }, []);
  ...
  const toggleRuns = async (workflowId: string) => {
    ...
    if (!isExpanded && !runsMap[workflowId]) {
      setRunsMap((prev) => ({
        ...prev,
        [workflowId]: { runs: [], skip: 0, allLoaded: false, loading: true },
      }));
      try {
        const runs = await fetchRuns(workflowId, 0, 10);
        setRunsMap((prev) => ({
          ...prev,
          [workflowId]: { runs, skip: 10, allLoaded: runs.length < 10, loading: false },
        }));
      } catch {
        setRunsMap((prev) => ({
          ...prev,
          [workflowId]: { runs: [], skip: 10, allLoaded: true, loading: false },
        }));
      }
    }
  };

  const loadMoreRuns = async (workflowId: string) => {
    const state = runsMap[workflowId];
    if (!state || state.loading || state.allLoaded) return;

    setRunsMap((prev) => ({
      ...prev,
      [workflowId]: { ...prev[workflowId], loading: true },
    }));

    try {
      const moreRuns = await fetchRuns(workflowId, state.skip, 10);
      setRunsMap((prev) => ({
        ...prev,
        [workflowId]: {
          runs: [...prev[workflowId].runs, ...moreRuns],
          skip: state.skip + 10,
          allLoaded: moreRuns.length < 10,
          loading: false,
        },
      }));
    } catch {
      setRunsMap((prev) => ({
        ...prev,
        [workflowId]: { ...prev[workflowId], loading: false, allLoaded: true },
      }));
    }
  };

  const invalidateRuns = useCallback((workflowId: string) => {
    setRunsMap((prev) => {
      if (!prev[workflowId]) return prev;
      return {
        ...prev,
        [workflowId]: { ...prev[workflowId], runs: [], skip: 0, allLoaded: false },
      };
    });
  }, []);
        ...
        {workflows?.map((w: any) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    <button
                      ...
                      className={`p-2 ${isExpanded ? "text-blue-600" : "text-gray-500 hover:text-blue-600"}`}
                      title={isExpanded ? "收起运行记录" : "查看运行记录"}
                    >
                      {isExpanded ? (
                        <ChevronDown size={16} />
                      ) : (
              ...
              {isExpanded && (
                <div className="mx-5 mb-4 pt-4 border-t">
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        {runsState?.runs.length === 0 && (
                          <p className="text-sm text-gray-400 py-2">暂无运行记录</p>
                        )}
                      </div>
                      {runsState && !runsState.allLoaded && (
                        <div className="mt-3 pt-3 border-t flex justify-center">
                          <button
                            onClick={() => loadMoreRuns(w.id)}
